...
# Make a KDTree for fast search of nearest neighbours
    ...
    _tree = sp.cKDTree(data_locs, leafsize=16, compact_nodes=True, copy_data=False, balanced_tree=True)
    ...
    for iy in range(0,ny):
        ...
        for ix in range(0,nx):
            xloc = xmn + (ix-0)*xsiz
            current_node = (yloc,xloc)
            tree[current_node] = _tree.query(current_node, ndmax, distance_upper_bound=radius)

    kmap, vmap = kb2d_jit_main(
        tree, nd, x, y, vr,
        xcol, ycol, vcol, tmin, tmax, nx, xmn, xsiz, ny, ymn, ysiz, nxdis, nydis, ndmin,
        ndmax, radius, ktype, skmean, vario_int, vario_float)
